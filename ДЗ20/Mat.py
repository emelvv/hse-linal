...
class Mat():
        ...
        if self._type == "mat":
            for i in range(len(arr)):
                for j in range(len(arr[0])):
                    arr[i][j] = sympy.simplify(arr[i][j])
    ...
    def __call__(self, variable, value):
        _new_arr = deepcopy(self._arr)
        for i in range(len(self._arr)):
            for j in range(len(self._arr[0])):
                _new_arr[i, j] = self._arr[i, j].subs(variable, value)
        return Mat(np.array(_new_arr))
