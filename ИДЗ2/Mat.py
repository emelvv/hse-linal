...
class Mat():
    __slots__ = ["_arr", "_slu", "_type", "brac"]
    # types:
    # mat, perm

    def __init__(self, arr: np.ndarray, slu: int = -1, type="mat", brac=False):
        self._slu = slu
        self._type = type
        self.brac = brac
            ...
            for i, x in enumerate(self._arr):
                for j, y in enumerate(x):
                    if j == self._slu:
                        s += "| "
                    s += ("(" if self.brac else "") + \
                        str(sympy.simplify(y))+(")" if self.brac else "")+" "
                s += "\n"
            return s
        if self._type == "perm":
            s = ""
    ...
    def check_slu_1(self, *xs):
        ...
        for row in self._arr:
            ...
            for i in range(len(xs)):
                s += xs[i]*row[i]
            if sympy.simplify(s) != sympy.simplify(row[-1]):
                return False
        return True
    ...
    def random(n, m=-1,  type="Mat"):
        ...
        if type == "Mat":
            if m == -1:
                return Mat(np.random.randint(-100, 100, (n, n)))
            else:
                return Mat(np.random.randint(-100, 100, (n, m)))
